...
## 2026-05-16 - Explicit ARIA Labels for Minimalist Inputs
**Learning:** In minimalist designs where inputs (`<input>`, `<textarea>`) rely solely on placeholder text for visual context and lack explicitly associated `<label>` elements, screen readers often fail to announce their purpose, leaving visually impaired users with context-free editable fields. Additionally, applying `aria-label` to inputs wrapped inside existing `<label>` tags (e.g., custom checkboxes) destructively overrides the text content of the parent label.
**Action:** Always add `aria-label` directly to text-based inputs that don't have visible labels, but avoid adding it to inputs nested inside explicit parent `<label>` tags to prevent breaking screen reader context.
