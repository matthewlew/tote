...
## 2026-03-21 - Labels for Placeholder-Only Inputs
**Learning:** Inputs and textareas that rely exclusively on `placeholder` attributes for visual context (like the import link and paste fields) are poorly supported by many screen readers, causing them to be announced simply as "edit" without context. This makes critical onboarding flows inaccessible.
**Action:** Always provide an explicit `aria-label` that succinctly describes the expected input on any `<input>` or `<textarea>` element that lacks a dedicated, programmatic `<label>`.
