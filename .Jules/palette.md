...
## 2026-05-08 - Accessible Keyboard Shortcuts and Decorative Arrows
**Learning:** Visual keyboard shortcut hints (like `<span class="shortcut">l</span>`) and decorative structural arrows (like `<span>→</span>` or `<span class="arr">→</span>`) create confusing, stuttering output for screen reader users when left exposed in the DOM (e.g., a button reading "Enable location l right arrow").
**Action:** Always add `aria-hidden="true"` to inline visual shortcut hints, decorative arrows, and bullet separators. To maintain feature parity for visually impaired users, apply the corresponding `aria-keyshortcuts="<key>"` attribute directly to the parent interactive element.
