...
## 2026-03-18 - Escape Key Overlay Dismissal and Screen History
**Learning:** Keyboard users expect the Escape key to consistently dismiss overlays, active details, and navigate back through modal screens. Without tracking screen history, pressing Escape to navigate backwards from a deep flow results in dead-ends or abrupt closures instead of stepping back logically.
**Action:** Always implement explicit Escape key handling in global keydown listeners, prioritizing the most specific open state first (e.g., expanded rows > collection details > full-screen modals). Track screen navigation history in an array stack (ignoring loading states) to enable logical backward traversal with the Escape key. Provide visual keyboard shortcut hints (`<span class="shortcut">esc</span>`) on corresponding "Back" or "Close" buttons.
