...
## 2026-03-18 - Explicit Escape handling and ARIA Key Shortcuts
**Learning:** Visual shortcuts (like "esc" or "l") and implied keyboard navigation patterns often lack parity with the accessibility tree, leaving screen reader users unaware of functionality. Relying purely on document-level keydown listeners without managing `aria-keyshortcuts` or explicitly handling the UI stack can create dead ends or unexpected dismissals for both keyboard-only and assistive tech users.
**Action:** Always pair visual shortcut indicators with `aria-keyshortcuts` on the parent interactive element. When hiding visual cues from screen readers (e.g., `aria-hidden="true"`), ensure the information is conveyed through standard attributes. Implement explicit `Escape` handling in global keydown listeners to systematically dismiss overlays from foreground to background, and consistently return keyboard focus to the originating trigger element using a stack like `st.focusHistory`.
