...
## 2026-03-24 - Screen Reader Verbosity with Inline UI Shortcuts
**Learning:** Adding visual keyboard shortcut indicators directly into interactive button labels (e.g., `<button>All <span class="shortcut">1</span></button>`) creates a confusing experience for screen reader users, who hear them read sequentially (e.g., "All 1 button"). This mixes semantic meaning with structural visual cues.
**Action:** When adding visual keyboard shortcut hints, always add `aria-hidden="true"` to the visual hint element, and explicitly attach an `aria-keyshortcuts` attribute to the parent interactive element (e.g., the button) to guarantee equivalent experience for visually impaired users without verbal clutter.
