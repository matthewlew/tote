...
## 2026-03-18 - Input Field Accessibility & Focus States
**Learning:** Placeholder-only input fields (like the URL and Paste fields in the Import tab) lack accessible names for screen readers unless explicitly labeled. Furthermore, disabling `outline` globally or on inputs (e.g., `outline: none;`) to achieve a custom design removes default browser focus rings, leaving keyboard users completely unable to track their position.
**Action:** Always provide an explicit `aria-label` for inputs that lack visible `<label>` elements. When overriding default browser outlines for aesthetic reasons, always reinstate a `:focus-visible` rule (e.g., matching the app's standard `var(--bd)` border styling) to ensure keyboard navigation remains visibly traceable.
