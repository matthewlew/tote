...
## 2026-03-22 - Graceful Overlay Dismissal via Escape Key
**Learning:** In complex web apps relying heavily on custom overlay states (like modals, expanded list items, and full-screen viewports), not providing an `Escape` key mapping breaks the fundamental user expectation of standard OS window management. Users intuitively press `Escape` to close the topmost open element. If pressing `Escape` doesn't work or causes an unintended global reset rather than an ordered backtracking (e.g. tracking `prevScreen`), it severely disrupts the interaction flow.
**Action:** Always implement explicit, prioritized `Escape` key handling in the global `keydown` listener to dismiss overlays from most specific (e.g. expanded row) to least specific (e.g. full screen picker), track the previous view state (`prevScreen`) to prevent navigation dead-ends, and explicitly surface these keyboard shortcuts visually (`<span class="shortcut">esc</span>`) on the corresponding dismiss buttons to promote discovery.
