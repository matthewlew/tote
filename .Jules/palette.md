...
## 2026-03-18 - Sequential Overlay Dismissal & Screen History
**Learning:** Hardcoded "Back" buttons in overlays like the neighborhood picker cause users to lose their state context if they entered from somewhere unexpected (e.g. from the app header instead of initial onboarding). Furthermore, missing `Escape` key support for overlays traps keyboard-only users.
**Action:** Implement an array-based history stack (`st.screenHistory`) to track previous screens dynamically. Add sequential `Escape` key handling that dismisses foreground elements first (expanded list items) before background overlays (modals), and provide visual shortcut hints (`<span class="shortcut" aria-hidden="true">esc</span>`) on dismissal buttons.
