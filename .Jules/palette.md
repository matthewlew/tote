...
## 2026-05-16 - ARIA attributes for visual shortcuts and labels
**Learning:** Adding `aria-hidden="true"` to structural arrows and bullet separators in text, as well as `aria-keyshortcuts` to elements with keyboard bindings, provides parity for screen reader users and cleans up the accessibility tree.
**Action:** Always verify if visual embellishments (like "→") need hiding from screen readers, and if elements with keyboard shortcuts correctly announce those shortcuts using `aria-keyshortcuts`. Ensure input fields without explicit `<label>` elements use `aria-label`.
