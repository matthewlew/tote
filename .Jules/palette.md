...
## 2026-03-18 - Visual Shortcuts vs Keyboard Access
**Learning:** To prevent screen readers from reading out confusing, combined text from UI elements, inline visual keyboard shortcut indicators, structural arrows (e.g., `<span class="arr">→</span>`), and bullet separators must be explicitly hidden from the accessibility tree. When these visual indicators (e.g., `<span class="shortcut">1</span>`) are inside interactive elements, screen readers announce them confusingly as part of the button text.
**Action:** Always apply `aria-hidden="true"` to visual shortcut indicators, structural arrows, and bullet separators. Furthermore, when hiding visual shortcut indicators, always pair this by adding `aria-keyshortcuts="<key>"` to the parent interactive element (like `<button>`) to ensure feature parity for visually impaired users.
