...
## 2026-03-18 - Screen Reader Verbosity from Inline Visual Hints
**Learning:** Screen readers blindly parse inline structural or visual spans like `<span class="arr">→</span>` or keyboard shortcut hints like `<span class="shortcut">1</span>`, reading out confusing combined text (e.g. "Enable location l rightwards arrow") that destroys context.
**Action:** Always apply `aria-hidden="true"` to structural arrows and keyboard shortcut hints inside interactive elements so assistive technologies read out only the clean, semantic label.
