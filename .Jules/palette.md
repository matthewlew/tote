...
## 2024-04-02 - Hide visual keyboard shortcuts from screen readers
**Learning:** In keyboard-centric apps, visual indicators for shortcuts (like `<span>l</span>`) are often placed directly inside buttons. Screen readers will read these linearly (e.g., "Enable location l button"), confusing users.
**Action:** Always apply `aria-hidden="true"` to structural/visual keyboard shortcut indicators that duplicate or confuse the semantic name of the interactive element.
