...
## 2026-03-18 - Escape Key Overlay Handling
**Learning:** For UX/accessibility, overlays and modals (like expanded item details, neighborhood pickers, etc.) must be dismissable using the `Escape` key. Without this, keyboard-only users are trapped in the overlay state. Furthermore, when dismissing an overlay, keyboard focus must be explicitly returned to the element that triggered it (e.g. the list row or the button), so the user's reading flow is not lost.
**Action:** Always implement explicit 'Escape' key handling in global keydown listeners to consistently dismiss overlays. Safely ignore `Escape` events when focus is inside `INPUT` or `TEXTAREA` elements to prevent accidental dismissals during form interaction. Ensure the evaluation order checks the top-most foreground layers first, and return keyboard focus to the originating trigger element upon dismissal. Provide visual shortcut hints (`<span class="shortcut">esc</span>`) on the corresponding 'Back' or 'Close' buttons.
